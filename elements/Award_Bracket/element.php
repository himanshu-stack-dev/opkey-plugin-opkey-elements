...
class Awardbracket extends \Breakdance\Elements\Element
{
    ...
    {
        return ['0' =>  ['title' => 'Adjust column widths','inlineScripts' => ['(function() {
  const root = document.querySelector(\'%%SELECTOR%%\');
  if (!root) return;

  const columns = root.querySelectorAll(\'.columns > .column\');
  const partners = root.querySelector(\'.partners\');

  if (partners && partners.children.length === 1) {
    columns[0].style.flex = \'2 0 0\';
  } else {
    columns[0].style.flex = \'\';
  }
})();
'],],];
    }
